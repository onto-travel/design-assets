  ...
  function init(userOpts){
    ...
    function apply(){
      ...
      if(opts.validate==='generic'){
        ...
        scoreEl.innerHTML = '<b class="vd-rev-num">94%</b><span class="vd-rev-slash">of guests recommend it</span>';
        relGroups.innerHTML = '<ul class="vd-chips">'
          + ['Beachfront setting','Standout service','6 restaurants','Lush gardens'].map(c=>chipLi(c)).join('')
          + '</ul>';
      } else {
        const chosen = knownIn();
        scoreEl.textContent = scoreFor(chosen, heads()) + '% match for this trip';
        const pills = [];
